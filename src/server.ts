...
export default {
  async fetch(request: Request, env: Env, _ctx: ExecutionContext) {
    ...
    if (url.pathname === "/check-google-api-key") {
    ...
    if (!process.env.GOOGLE_API_KEY) {
      console.error(
        "GOOGLE_API_KEY is not set, don't forget to set it locally in .dev.vars, and use `wrangler secret bulk .dev.vars` to upload it to production"
      );
    }
    return (
